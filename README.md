...
The processed matrices in `data/` are the released paper-analysis inputs and contain no direct identifiers. The original, unprocessed data used to create them are available from the following public sources.

| Analysis | Original data source | Processed input used by this release |
|---|---|---|
| MNIST | [MNIST handwritten digits](http://yann.lecun.com/exdb/mnist/) | `data/mnist/mnist_k2.npz` and `data/mnist/mnist_k16.npz` |
| Mouse-DS | [Mice Protein Expression, UCI dataset 342](https://archive.ics.uci.edu/dataset/342/mice%2Bprotein%2Bexpression) | `data/mouse_ds/Data_Cortex_Nuclear.csv` |
| Leukemia | 10x Genomics AML027 [pre-transplant](https://www.10xgenomics.com/datasets/aml-027-pre-transplant-bmm-cs-1-standard-1-1-0) and [post-transplant](https://www.10xgenomics.com/datasets/aml-027-post-transplant-bmm-cs-1-standard-1-1-0) bone-marrow mononuclear cells | `data/leukemia/target_prepost_3000hvg.h5ad` and `data/leukemia/background_healthy_3000hvg.h5ad` |
| McFarland | [MIX-seq single-cell perturbation data](https://doi.org/10.6084/m9.figshare.10298696.v1) from McFarland *et al.* | `data/mcfarland/mcfarland_preprocessed.h5ad` |
| MDD | [GEO GSE144136](https://www.ncbi.nlm.nih.gov/geo/query/acc.cgi?acc=GSE144136), single-nucleus RNA-seq of dorsolateral prefrontal cortex from MDD cases and controls | `data/mdd/mdd_target_1147x3000.h5ad` |
| Synthetic gene-expression benchmark | Generated by `demo/generate_simulation.py`; no external source dataset | `demo/data/beta_*_seed_0.npz` |

Please cite the original data providers when using these datasets. `Source_Data.xlsx` is a separate Nature Communications source-data file: it contains the numerical values underlying the reported figures and tables, not the original datasets.
